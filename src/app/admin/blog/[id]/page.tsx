...
export default function EditBlogPostPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchPost = async () => {
    try {
      const res = await fetch(`/api/admin/blog/${id}?t=${Date.now()}`, { cache: "no-store" });
  ...
  if (error) {
    return (
      <div className="max-w-4xl mx-auto flex flex-col items-center text-center py-20">
        ...
        <h2 className="text-xl font-black text-navy mb-2">Error Loading Post</h2>
        <p className="text-sm font-medium text-gray-500 mb-6">{error}</p>
        <Link href="/admin/blog" className="px-6 py-2.5 bg-navy text-white rounded-xl text-sm font-bold hover:bg-navy/90 transition-colors">
          Back to Blog
        </Link>
      </div>
    );
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black text-navy">Edit Post</h1>
            <p className="text-sm font-medium text-gray-500 mt-1">Update the details of your story.</p>
          </div>
        </div>
        <button 
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm space-y-4">
            ...
            {formData.featuredImage && (
              <div className="mt-4 rounded-xl overflow-hidden border border-gray-200 h-40 relative bg-gray-100">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm space-y-4">
            ...
            <div>
              ...
              <input 
                ...
                className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm font-medium outline-none focus:bg-white focus:border-coral/50 focus:ring-2 focus:ring-coral/10 transition-all"
